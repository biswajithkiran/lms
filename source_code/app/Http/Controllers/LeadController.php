...
class LeadController extends Controller
{
    ...
    public function save_incomplete_lead(Request $Request)
    {
        $ip_address             = $Request->ip();
        $session_id             = session()->getId();
        $leads                  = Lead::where('lead_id', '=', $session_id)
                                        ->where('status','=','I')
                                        ->get();
        ...
        {
            $lid                        = $leads[0]->id;
            $arrLead                    = Lead::find($lid);
            $arrLead->first_name        = $Request->first_name;
            $arrLead->last_name         = $Request->last_name;
            $arrLead->email             = $Request->email;
            $arrLead->phone             = $Request->phone;
            $arrLead->home_area         = $Request->home_area;
            $arrLead->address           = $Request->address;
            $arrLead->status            = 'I';
            $arrLead->save();
        }
    }
